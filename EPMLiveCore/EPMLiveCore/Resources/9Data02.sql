...
if not exists (select jobtype_id from TIMERJOBTYPES where jobtype_id = 150)
begin
    INSERT INTO TIMERJOBTYPES (jobtype_id,NetAssembly,NetClass,[Title],priority) VALUES (150,'EPM Live Core, Version=1.0.0.0, Culture=neutral, PublicKeyToken=9f4da00116c38ec5','EPMLiveCore.Jobs.Wizard','Setup Wizard',9)
end
else
begin
...
begin
    UPDATE INT_MODULES SET NetAssembly='UplandIntegrations, Version=1.0.0.0, Culture=neutral, PublicKeyToken=9f4da00116c38ec5', NetClass='UplandIntegrations.FileBound.Integrator', [Title]='FileBound', Description='Filebound will allow you to connect your lists to an advanced workflow system.', Icon='fb.png', CustomProps='<Properties><Connection><Input Type="Text" Property="SiteUrl" Title="FileBound Site URL" /><Input Type="Text" Property="APIUrl" Title="FileBound API URL" /><Input Type="Text" Property="Username" Title="Username" /><Input Type="Password" Property="Password" Title="Password" /></Connection><General><Input Type="Select" Property="Folder" Title="Select a folder" /><Input Type="Select" Property="DataType" Title="Select data type" /><Input Type="Password" Property="RelatedAssign" Title="Assignment Integration Key" >When selecting the Item data type, enter the integration key here for the associated assignments list integration.</Input><Input Type="Select" Property="UserMapType" Title="Select the user mapping field" /></General></Properties>', AvailableOnline='True',INT_CAT_ID='599c89b2-6330-4e54-aea5-00185f20cce0' where MODULE_ID='a0950b9b-3525-40b8-a456-7403156dc490'
end
