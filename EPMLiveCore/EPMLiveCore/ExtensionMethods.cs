...
namespace EPMLiveCore
{
    public static class ExtensionMethods
    {
        ...
        public static string GetContents(this SPFile viewAspxFile)
        {
            ...
            using (var fileStream = viewAspxFile.OpenBinaryStream())
            {
        ...
        {
            var htmlAsByteArray = new ASCIIEncoding().GetBytes(fileContents);
                ...
                default:
                    {
                        ...
                        else
                        {
                            ...
                            if (lastWeekTimeSpan.Days <= 0 && thisWeekTimeSpan.Days > 0)
                                friendlyDate = "Last " + dateTime.DayOfWeek.ToString() + " at " +
                            ...
                            else
                            {
                                ...
                                if (nowTimeSpan.Days > 0 && thisWeekTimeSpan.Days <= 0)
                                    friendlyDate = dateTime.DayOfWeek.ToString() + " at " +
                                                   new DateTime(dateTime.TimeOfDay.Ticks).ToString("hh:mm:ss tt");
                                else
                                {
                                    TimeSpan nextWeekTimeSpan = nextWeek - dateTime;
        ...
        {
            return webPartManager.WebParts.Cast<WebPart>().FirstOrDefault(webPart => webPart.GetType().ToString().ToLower().Contains(webPartType.ToLower()));
        }
