...
function Init() {
    (function ($$, $) {
                ...
                if (window._wePeopleEditorControlPropertyArray) {
                    for (var k in window._wePeopleEditorControlPropertyArray) {
                        ...
                        $('#' + controlProps.wePeopleEditorDivID).blur(function () {
                            ...
                            if ($(':focus').attr('class') == 'spnUserBlk') {
                                return;
                            }

                            var newUsers = $('#' + controlProps.wePeopleEditorDivID).text().trim();
                            if (newUsers != '' &&
                                ($('#' + controlProps.autoCompleteDivID).css('display') == 'none' || $('#' + controlProps.autoCompleteDivID).css('display') == undefined) &&
                                $('#divSavingUser').css('display') != 'none') {
                                $('#' + controlProps.wePeopleEditorDivIDRoot + "_browse").css('display', 'none');
                                $('#' + controlProps.wePeopleEditorDivIDRoot + "_checkNames").css('display', 'none');
                                ...
                                var postData = { users: newUsers, listid: $('#' + hdnListId).val(), itemid: $('#' + hdnItemId).val(), weburl: curWebUrl };
                                $.post(window.curWebURL + "/_layouts/epmlive/commentersupdate.aspx", postData, function (data) {
                                    if (data.trim() == 'success') {

                                        $('#imgLoading').fadeOut('slow');
                                        $('#divSavingUser').fadeOut('slow');
                                        $('#' + controlProps.wePeopleEditorDivID).html(tempHtml);
                                    }
                ...
                if (oJson.Result.Team) {
                    for (var index in oJson.Result.Team) {
                        ...
                        if (cleanItemTitle.indexOf(cleanSearchText) == 0 &&
                            ...
                            !ArrayContains(authorid.split(','), userId) &&
                            !ArrayContains(assigneeids.split(','), userId)) {
                            hasMatch = true;
                            $('#' + controlProps.autoCompleteDivID).append("<div class='autoText' username='" + itemUsername + "' title='Username: " + itemUsername + " | Email: " + userEmail + "' id='autoText_" + itemIndex + "' userid='" + userId + "'>" + itemTitle + "</div>");
                            itemIndex++;
                            continue;
                        }
        ...
        function UpdateSingleSelectPickerValueWOValidation(controlProps, newTextVal, index) {
            ...
            var spEntityStyle = '<span contenteditable="false" title="' + newTextVal + '" class="ms-entity-resolved" tabindex="-1" iscontenttype="true" id="span' + index + '" onmouseover="this.contentEditable=false;" onmouseout="this.contentEditable=true;">' +
	                                '<div description="' + newTextVal + '" isresolved="True" displaytext="' + newTextVal + '" key="' + index + '" id="divEntityData" style="display:none;">' +
		                                '<div style="display:none;" data="">' + newTextVal + '</div>' +
	                                '</div>' +
        ...
        function UpdateMultiSelectPickerValueWOValidation(controlProps, newTextVal, index) {
            ...
            $('#' + controlProps.wePeopleEditorDivID).empty();
            $('#' + controlProps.wePeopleEditorDivID).html(clean);
