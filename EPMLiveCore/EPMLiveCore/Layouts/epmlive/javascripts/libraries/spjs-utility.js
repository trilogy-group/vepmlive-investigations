 ...
 * Copyright (c) 2009-2012 Alexander Bautz (Licensed under the MIT X11 License)
 * LastMod: 07.03.2012
**********************************/
function doResizeDialog(){
	...
	if(dlgContent.attr('DragBehavior')!==null&&dlgContent.css('left')!=='10px'){
		...
		dlgOuterFrame.css({position:'absolute',top:top+"px",height:formH+'px'});
		dlgContent.css({position:'absolute',top:top+"px",height:formH+'px'});
...
function spjs_getItemByID(argObj){
	...
	qRes = spjs_QueryItems({'listName':argObj.listName,'listBaseUrl':argObj.listBaseUrl,'query':query,'viewFields':argObj.viewFields});
	if(qRes.count===0){
		return null;
	}else{
		return qRes.items[0];
...
function spjs_QueryItems(argObj){
	...
	content = spjs_wrapQueryContent({'listName':argObj.listName, 'query':argObj.query, 'viewName':argObj.viewName, 'viewFields':argObj.viewFields, 'rowLimit':argObj.rowLimit, 'pagingInfo':argObj.pagingInfo});
...
function spjs_wrapQueryContent(paramObj){
	...
	result.push('<ViewAttributes Scope="Recursive" />');
...
function setFieldValue(fin,newVal,onLoad){
	...
	var thisField, fieldType, optText, inputField, inputFieldHidden, split, newValueBuffer, id, friendlyVal;
	...
	switch(fieldType){
		...
		case 'SPFieldDateTime':
			...
			if(newVal[1]!==undefined){
				thisField.find('select:first').val(newVal[1]);
			}
		...
		break;
		default:
		alert("Unknown fieldType: "+fieldType+", please ensure you use the function init_fields_v2().");
	}
}
...
function getFieldValue(fin,dispform,multiValueJoinBy,optionalFilter){
	...
	var thisField, valRaw, fieldType, getFieldValueReturnVal, multiChoice, userMulti, lookupMulti, attrToFind, thisVal, link, descr, date, hour, minutes, AMPM, hourRaw, minutesRaw;
	thisField = $(fields[fin]);
	if(dispform==true){
		...
		switch(fieldType){
			...
			case 'SPFieldChoice':
				if(thisField.find('select').length>0){
					if(thisField.find('input:radio').length>0){
						if(thisField.find("input[id$='_FillInButton']:checked").length>0){
							getFieldValueReturnVal = thisField.find('input:last').val();
						}else{
					...
					if(thisField.find("input:radio:last:checked").length>0 && thisField.find('input:text').length>0){
						getFieldValueReturnVal = thisField.find('input:last').val();
					}else{
			...
			case 'SPFieldMultiChoice':
				...
				if(thisField.find("input:checkbox:last:checked").length>0 && thisField.find('input:text').length>0){
					multiChoice.pop();
					multiChoice.push(thisField.find('input:last').val());
				}
			...
			case 'SPFieldNote_EHTML':
				getFieldValueReturnVal = thisField.find("div[id$='_TextField_inplacerte']").html();
			break;
			case 'customHeading':
				getFieldValueReturnVal = '';
			break;
			default:
			getFieldValueReturnVal = "Unknown fieldType: "+fieldType+", please ensure you use the function init_fields_v2().";
		}
...
window.ExecuteOrDelayUntilScriptLoaded(improvizejQuery, 'jquery.min.js');
window.SP.SOD.notifyScriptLoadedAndExecuteWaitingJobs('spjs-utility.js');