    ...
    public class AdminFunctions : PFEBase
    {
                ...
                if (thisJob.NextRun <= dtTimeNow)
                {
                    switch (thisJob.FreqMode)
                    {
                        case 1:  // run only once
                            if (thisJob.LastRun == dtNull) selectedJob = thisJob;
                            break;
                        case 2:
                        case 3:
                        case 4:
                        case 5:
                        case 9:
                            if (thisJob.NextRun != dtNull || thisJob.FirstRun <= dtTimeNow) selectedJob = thisJob;
                            break;
                    }

                }
                if (selectedJob.UID != 0)
                    break;
            }
            reader.Close();


            // let's see if we have a job to process
            if (selectedJob.UID > 0)
            {
                ...
                SubmitJobRequest(dba, selectedJob.WResId, selectedJob.Context, selectedJob.Session, selectedJob.Comment, selectedJob.ContextData, basePath);
        ...
        public static bool CalcCategoryRates(DBAccess dba, out string sReply)
        {
            ...
            try
            {
                ...
                if (categoryrates.Count == 0)
                    return true;    //  no rates found - don't do anything - will leave old Periodic Rates as is
        ...
        public static bool CalcInternalAvailabilities(DBAccess dba, int calendar, string reslist)
        {
            ...
            try
            {
                ...
                if (reslist.Length > 0)
                {
                    ...
                    {
                        int nWresID;
                        if (int.TryParse(sList, out nWresID) == false)
                        {
                            bisValidResourceList = false;
                            break;
                        }
                    }
                }
                if (bisValidResourceList == false)
                    return false;
                ...
                if (reslist.Length > 0)
                    sCommand += " INNER JOIN dbo.EPG_FN_ConvertListToTable(N'" + reslist + "') LT on r.WRES_ID=LT.TokenVal";
                ...
                while (reader.Read())
                {
                    ...
                    if (oResource.StartDate < CalStart)
                        oResource.StartDate = CalStart;
                    if (oResource.FinishDate < oResource.StartDate || oResource.FinishDate > CalEnd)
                        oResource.FinishDate = CalEnd;
        ...
        public static bool CalcDefaultFTEs(DBAccess dba, int calendar, string catlist)
        {
            ...
            try
            {
                ...
                if (bisValidCatList == false || Categories.Count == 0)
                    return false;
                ...
                foreach (int lcal in calendars)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        int lConv = (int)(workhours.workhours(lWH, lHOL, dStartDate, dFinishDate, false) * 100);
        ...
        private static double GetPeriodRate(DateTime prdstart, int catID, List<CategoryRate> rates)
        {
            ...
            foreach (CategoryRate rateitem in rates)
            {
                ...
                else if (prdstart >= rateitem.EffectiveDate)
                    rate = rateitem.Rate;
                else if (prdstart < rateitem.EffectiveDate)
                    break;
            }
