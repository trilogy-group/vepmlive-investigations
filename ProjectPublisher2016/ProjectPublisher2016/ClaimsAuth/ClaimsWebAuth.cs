...
namespace ProjectPublisher2016
{
    class ClaimsWebAuth : IDisposable
    {
        ...
        public CookieCollection Show()
        {
            ...
            this.webBrowser.Navigate(this.TargetSiteUrl);
        ...
        private void GetClaimParams(string targetUrl, out string loginUrl, out Uri navigationEndUrl)
        {
            ...
            try
            {
                ...
                ExtraHeadersFromResponse(response, targetUrl, out loginUrl, out navigationEndUrl);
            }
            catch(WebException webEx)
            {
                if(webEx.Response == null)
                {
                    ...
                    ExtraHeadersFromResponse(webEx.Response, targetUrl, out loginUrl, out navigationEndUrl);
            }
        }

        private bool ExtraHeadersFromResponse(WebResponse response, string targetUrl, out string loginUrl, out Uri navigationEndUrl)
        {
            loginUrl = null;
        ...
        private void ClaimsWebBrowser_Navigated(object sender, WebBrowserNavigatedEventArgs e)
        {
            ...
            //url = url.Substring(0, lasti);

            if(fldNavigationEndUrl != null && e.Url.ToString().ToLower().StartsWith(TargetSiteUrl.ToLower()))
            {
                this.fldCookies = ExtractAuthCookiesFromUrl(this.LoginPageUrl);
                if(this.fldCookies != null && this.fldCookies["FedAuth"] != null)
