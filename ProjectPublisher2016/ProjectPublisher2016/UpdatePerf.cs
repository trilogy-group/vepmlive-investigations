	...
	public class UpdatePerf
	{
		...
		public static int getUpdateCount(Microsoft.Office.Interop.MSProject.Project pj)
		{
            ...
            try
            {
                ...
                if (nd.Attributes["Status"].Value == "0")
                {
                    ...
                    if (ID != "")
                    {
                        frmStatus.label1.Text = "Getting Project Updates...";
                        frmStatus.Refresh();

                        docRet.LoadXml(oAPI.Execute("GetUpdateCount", "<Project ID=\"" + ID.Split('.')[2] + "\" PlannerID=\"" + Connect.getProperty("EPMLivePlanner", pj) + "\"/>"));

                        nd = docRet.FirstChild;

                        if (nd.Attributes["Status"].Value == "0")
                        {
                            int.TryParse(nd.InnerText, out updateCount);
                        }
                        else
                        {
                            MessageBox.Show("Error Getting Updates: " + nd.FirstChild.InnerText);
                        }
                    }
                }
                else
            ...
            catch (System.Exception ex)
            {
                ...
                MessageBox.Show("Error Getting Update Count:\n" + ex.Message + "\n\n" + ex.StackTrace);
            }
			frmStatus.Dispose();
			//MessageBox.Show("D6");
			return updateCount;
		...
		public static int update(Microsoft.Office.Interop.MSProject.Project pj,Microsoft.Office.Interop.MSProject.Application app)
		{
            ...
            if (nd.Attributes["Status"].Value == "0")
            {
                ...
                docRet.LoadXml(oAPI.Execute("GetUpdates", "<Project ID=\"" + ID.Split('.')[2] + "\" PlannerID=\"" + Connect.getProperty("EPMLivePlanner", pj) + "\"/>"));
                ...
                if (nd.Attributes["Status"].Value == "0")
                {
                    ...
                    foreach (XmlNode ndTask in nd.SelectNodes("//Task"))
                    {
                        Connect.Tasks newTask = getTask(ndTask, providerEn);
                        newTask.taskuid = ndTask.Attributes["UID"].Value.ToString();
                        newTasks.Add(ndTask.Attributes["UID"].Value.ToString(), newTask);
                    }
                    ...
                    foreach (DictionaryEntry entry in newTasks)
                    {
                        ...
                        try
                        {
                            if (nt.taskuid.Contains("."))
                            {
                                myHash.Add(nt.taskuid.Split('.')[0], pj.Tasks.get_UniqueID(int.Parse(nt.taskuid.Split('.')[0])));
                            }
                            else
                                myHash.Add(nt.taskuid, pj.Tasks.get_UniqueID(int.Parse(nt.taskuid)));
                        }
                        catch { }
                    ...
                    if (Connect.taskList.Length > 0)
                    {
                        ...
                        {
                            UpdatePerf.hshResFields = new Hashtable();
		...
		private static Connect.Tasks getTask(XmlNode nd, IFormatProvider providerEn)
		{
			string ListID = nd.Attributes["ItemID"].Value;
			string taskuid = "";
			string taskname = "";
			string notes = "";
			string resource = "";
			...
			try
			{
                taskuid = nd.Attributes["UID"].Value;
			}
			catch(System.Exception){}
			try
			{
				editor = nd.SelectSingleNode("Field[@Name='Editor']").InnerText;
				editor = editor.Replace(";#","\n").Split('\n')[1];
			}
			catch(System.Exception){}
			try
			{
                taskname = nd.SelectSingleNode("Field[@Name='Title']").InnerText;
			}
			catch(System.Exception){}
			try
			{
				notes = cleanUp(nd.SelectSingleNode("Field[@Name='Notes']").InnerText);
			}
			catch(System.Exception){}
			try
			{
				resource =nd.SelectSingleNode("Field[@Name='AssignedTo']").InnerText;
				resource = resource.Replace(";;",";");
			}
			catch(System.Exception){}
			try
			{
				milestone = nd.SelectSingleNode("Field[@Name='Milestone']").InnerText;
			}
			catch{}
			try
			{
				type = int.Parse(nd.SelectSingleNode("Field[@Name='IsAssignment']").InnerText);
			}
			catch(System.Exception){}
			try
			{
				pctComplete = (Convert.ToDouble(nd.SelectSingleNode("Field[@Name='PercentComplete']").InnerText, providerEn) * 100).ToString();
			}
			catch(System.Exception){}
			try
			{
                breadCrumbs = nd.SelectSingleNode("Field[@Name='TaskHierarchy']").InnerText;
			}
			catch(System.Exception){}
					
			try
			{
				modified = DateTime.Parse(nd.SelectSingleNode("Field[@Name='Modified']").InnerText).ToString();
			}
			catch{}
            ...
            {
                string field = ndField.Attributes["Name"].Value.ToUpper();
                if (field == "STARTDATE")
                    field = "START";
                if (field == "DUEDATE")
                    field = "FINISH";

                IDictionaryEnumerator e = Connect.hTaskCenterFields.GetEnumerator();
                while (e.MoveNext())
                {
                    if (e.Value.ToString().ToLower().Replace(" ", "").Contains("(" + field.ToLower() + ")") || e.Value.ToString().ToLower().Replace(" ", "").Equals(field.ToLower()))
                    {
                ...
                if (Connect.hTaskCenterFields.Contains(field))
                {
                    ...
                    try
                    {
                        iField = ((Connect.TaskField)Connect.hPprojectFields[field]).fieldId;
                    }
                    catch { }

                    //Connect.hPprojectFields 
                    ...
                    if (iField != 0)
                    {
                        if (tField.type != 9)
                        {
                            ...
                            string sFieldVal = ndField.InnerText;
        ...
        private static string getAttribute(XmlNode nd, string attr)
        {
            try
            {
                return nd.SelectSingleNode("Field[@Name='" + attr + "']").InnerText;
            }
            catch { }
            return "";
        }
				...
				if(tsk.Assignments.Count == 0)
				{
					...
					else
					{
						...
						if(field == "")
							field = "Number15";
						int fieldId = 0;
                        if (UpdatePerf.hshResFields.Contains(field))
						{
                            fieldId = (int)UpdatePerf.hshResFields[field];
						}
