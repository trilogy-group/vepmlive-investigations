...
{
	internal static class InnovationServiceApplication
	{
		internal static Type ServiceApplicationType
		{
			get
			{
                return Utilities.Types.TryGetType(NGTypes.Innovation.InnovationServiceApplicationTypeFullName, NGAssemblies.Innovation.NewsGatorInnovationServiceLibrary);
			}
		}
		...
		{
            return Utilities.ServiceApplication.FindServiceInstanceWhereOrDefault(x => x.GetType().FullName == NGTypes.Innovation.InnovationServiceApplicationTypeFullName);
		}
