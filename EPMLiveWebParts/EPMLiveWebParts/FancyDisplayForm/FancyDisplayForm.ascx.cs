    ...
    public partial class FancyDisplayForm : WebPart
    {
        ...
        private void LoadFancyFormData()
        {
            ...
            try
            {
                ...
                if (list.EnableVersioning && SPContext.Current.ListItemVersion != null)
            ...
            {
                lblItemTitle.Text = HttpUtility.HtmlEncode(Convert.ToString(item["Title"]));
                ...
                if (fieldProperties != null)
                {
                    foreach (SPField field in list.Fields)
                    {
                        try
                        {
                            ...
                            {
                                //Fetch FieldValue From Latest SPListItem Version
                                if (isLatestVersion)
                                {
                                    fieldValue = Convert.ToString(item[field.InternalName]);
                                }
                                else
                                {
                                    ...
                                    foreach (SPListItemVersion itemVersion in item.Versions)
                                    {
                                        ...
                                        {
                                            fieldValue = Convert.ToString(itemVersion[field.InternalName]);
                                        }
                                    }
                                }
                                    ...
                                    switch (field.Type)
                                    {
                                        case SPFieldType.DateTime:
                                            if (!string.IsNullOrEmpty(fieldValue))
                                                FillDateDetailsSection(field.Title, GetFormattedDate(Convert.ToDateTime(fieldValue)));
                                            else
                                                FillDateDetailsSection(field.Title, string.Empty);
                                            break;
                                        case SPFieldType.User:
                                            FillPeopleDetailsSection(field.Title, fieldValue);
                                            break;
                                        case SPFieldType.Note:
                                            FillNarrativeDetailsSection(field.Title, field.GetFieldValueAsHtml(fieldValue), item, ((SPFieldMultiLineText)field).AppendOnly);
                                            break;
                                        default:
        ...
        private void FillNarrativeDetailsSection(string fieldName, string fieldValue, SPListItem item, bool appendOnly)
        {
            ...
            if (narrativeDetailsFieldsCount < NARRATIVE_DETAILS_SECTION_FIELD_COUNT)
            {
                ...
                if (appendOnly)
                {
                    sbNarrativeDetailsContent.Append("<td style='width:auto;'>");
                    ...
                    if (isLatestVersion)
                    {
                        if (item.Versions.Count > 0)
                        {
                            ...
                            {
                                if (!string.IsNullOrEmpty(Convert.ToString(item.Versions[i][fieldName])))
                                {
                                    sbNarrativeDetailsContent.Append("<div><span class=\"ms-noWrap\"><span class=\"ms-spimn-presenceLink\">" +
                                    "<span class=\"ms-spimn-presenceWrapper ms-imnImg ms-spimn-imgSize-10x10\">" +
                                    "<img class=\"ms-spimn-img ms-spimn-presence-disconnected-10x10x32\" src=\"/_layouts/15/images/spimn.png?rev=23\" alt=\"\"></span></span>" +
                                    "<span class=\"ms-noWrap ms-imnSpan\"><span class=\"ms-spimn-presenceLink\">" +
                    ...
                    else
                    {
                        ...
                        foreach (SPListItemVersion itemVersion in item.Versions)
                        {
                            //Display RTF field value till the version selected by the user.
                            if (itemVersion.VersionId <= SPContext.Current.ListItemVersion.VersionId)
                            {
                                if (!string.IsNullOrEmpty(Convert.ToString(itemVersion[fieldName])))
                                {
                                    sbNarrativeDetailsContent.Append("<div><span class=\"ms-noWrap\"><span class=\"ms-spimn-presenceLink\">" +
                                    "<span class=\"ms-spimn-presenceWrapper ms-imnImg ms-spimn-imgSize-10x10\">" +
                                    "<img class=\"ms-spimn-img ms-spimn-presence-disconnected-10x10x32\" src=\"/_layouts/15/images/spimn.png?rev=23\" alt=\"\"></span></span>" +
                                    "<span class=\"ms-noWrap ms-imnSpan\"><span class=\"ms-spimn-presenceLink\">" +
                                    "<img class=\"ms-hide\" src=\"/_layouts/15/images/blank.gif?rev = 23\" alt=\"\">" +
                                    "</span><a class=\"ms-subtleLink\" onclick=\"GoToLinkOrDialogNewWindow(this); return false;\" href=" + SPContext.Current.Web.Url + "/_layouts/15/userdisp.aspx?ID" + itemVersion.CreatedBy.LookupId + ">" + itemVersion.CreatedBy.User.Name + "</a></span></span> (" + itemVersion.Created + "): " + Convert.ToString(itemVersion[fieldName]) + "</div>");
                                }
                            }
                        }
                        sbNarrativeDetailsContent.Append("</div>");
                    ...
                    sbNarrativeDetailsContent.Append("</td>");
                }
                else
                {
            ...
            else
            {
                ...
                if (appendOnly)
                {
                    sbNarrativeDetailsShowAllRegion.Append("<td style='width:auto;'>");
                    ...
                    if (isLatestVersion)
                    {
                        if (item.Versions.Count > 0)
                        {
                            sbNarrativeDetailsShowAllRegion.Append("<div>");
                            for (int i = 0; i < item.Versions.Count; i++)
                            {
                                if (!string.IsNullOrEmpty(Convert.ToString(item.Versions[i][fieldName])))
                                {
                                    sbNarrativeDetailsShowAllRegion.Append("<div><span class=\"ms-noWrap\"><span class=\"ms-spimn-presenceLink\">" +
                                    "<span class=\"ms-spimn-presenceWrapper ms-imnImg ms-spimn-imgSize-10x10\">" +
                                    "<img class=\"ms-spimn-img ms-spimn-presence-disconnected-10x10x32\" src=\"/_layouts/15/images/spimn.png?rev=23\" alt=\"\"></span></span>" +
                                    "<span class=\"ms-noWrap ms-imnSpan\"><span class=\"ms-spimn-presenceLink\">" +
                                    "<img class=\"ms-hide\" src=\"/_layouts/15/images/blank.gif?rev = 23\" alt=\"\">" +
