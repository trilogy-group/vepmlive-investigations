    ...
    public class GridListView : Microsoft.SharePoint.WebPartPages.WebPart, IWebPartPageComponentProvider
    {
        ...
        */
        ...
        private string GetViews()
        {
            ...
            string currentViewUrl = string.Empty;

            string requestUrl = Convert.ToString(Page.Request.Url);
            // Get current view url
            if (requestUrl.Contains("/Lists"))
                currentViewUrl = requestUrl.Substring(requestUrl.IndexOf("/Lists")).TrimStart('/');
            ...
            if (propBagData != null)
            {
                ...
                #endregion
                splitViewcollection = splitViewcollection.OrderBy(a => a).ToList();
            ...
            else
            {
                ...
                {
                    vm.Add(new ViewsModel() { Title = view.Title, ServerRelativeUrl = view.ServerRelativeUrl });
                }
                vm = vm.OrderBy(a => a.Title).ToList();
                // getting view from list.Views
                foreach (ViewsModel view in vm)
                {
