                ...
                if (maxThreads > 0)
                {
                    ...
                    foreach (SPWebApplication webApp in SPWebService.ContentService.WebApplications)
                    {
                        ...
                        if (sConn != "")
                        {
                            SqlConnection cn = new SqlConnection(sConn);
                            cn.Open();
                            SqlCommand cmd = new SqlCommand("SELECT TOP " + maxThreads + " queueuid,timerjobuid,siteguid,webguid,listguid,itemid,jobtype,jobdata,userid,netassembly,netclass,title,[key] from vwQueueTimer where status=0 order by priority,dtcreated asc", cn);
