    ...
    public class HighTimerClass
    {
        ...
        private void logMessage(string type, string module, string message)
        {
            lock (thisLock)
            {
                ...
                using (StreamWriter swLog = new StreamWriter(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + "\\LOGS\\HTIMERLOG_" + dt.Year + dt.Month + dt.Day + ".log", true))
                {
