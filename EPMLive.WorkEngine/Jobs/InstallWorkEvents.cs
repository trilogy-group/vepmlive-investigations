...
namespace WorkEnginePPM.Jobs
{
    public class InstallWorkEvents : EPMLiveCore.API.BaseJob
    {
        ...
        {
            ArrayList worklists = null;
            SPWebCollection webColl = null;
            SPList oParentList = null;
            XmlDocument DocRet = null;
            try
            {
                 oParentList = web.Lists[base.ListUid];

                worklists = new ArrayList(EPMLiveCore.CoreFunctions.getConfigSetting(site.RootWeb, "epk" + oParentList.Title.Replace(" ", "") + "_worklists").Split('|'));
