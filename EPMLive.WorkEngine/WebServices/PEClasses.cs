...
namespace WorkEnginePPM
{
    ...
    internal class dbaUsers
    {
     ...
     //       oCommand.CommandText = cmdText;
            ...
            foreach (KeyValuePair<int, WorkEnginePPM.EPKCustomField> oCustomField1 in clnCustomFields)
            {
    ...
    internal class dbaCCV
    {
        public static StatusEnum CalculateCostValues(DBAccess dba, int nCTID, int nCBID, int nProjectID, out string sResult)
        {
            ...
            Dictionary<int, CostCategory> costcategories = new Dictionary<int, CostCategory>();  // DIC because I want foreach to get back in order put in
            ...
            foreach (KeyValuePair<int, CostCategory> costcategoryentry in costcategories)
            {
                CostCategory costcategory = costcategoryentry.Value;
            ...
            if (!bsingleUOM)  // if only one UOM can always roll up
            {
                for (int i = lCategoryCount - 1; i >= 0; i--)
                {
                    ...
                    CostCategory costcategory = costcategories[categoryUID];
            ...
            foreach (int ProjectID in listPIs)
            {
                ...
                if (dbaUsers.ExecuteSQLSelect(cmd, out reader) == StatusEnum.rsSuccess)
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            CostCategory costcategory = costcategories[category];
                ...
                for (int i = lCategoryCount - 1; i >= 0; i--)
                {
                    ...
                    {
                        CostCategory parent_costcategory = costcategories[costcategory.ParentUID];
