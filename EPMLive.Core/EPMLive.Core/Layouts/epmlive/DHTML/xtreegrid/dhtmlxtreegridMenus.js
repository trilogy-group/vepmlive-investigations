...
dhtmlXGridObject.prototype.gridactioncallback = function (dialogResult, returnValue) {
    //if (dialogResult == 1) {
        ...
        this.getSingleItem(siteid, webid, listid, itemid, curRow);
    //}
}

dhtmlXGridObject.prototype.getSingleItem = function(siteid,webid,listid,itemid,rowid)
{

	var val = SP.UI.Notify.addNotification("Updating Saved Row...", false,  "", null);

	dhtmlxAjax.post(this._singleItemUrl, "siteid=" + siteid + "&webid=" + webid + "&listid=" + listid + "&itemid=" + itemid + "&rowid=" + rowid, this.getSingleItemResponse);
}
...
dhtmlXGridObject.prototype.menuaction = function (obj, action, actiontype) {
    ...
    weburl = weburl + "/_layouts/epmlive/gridaction.aspx?action=" + action + "&webid=" + webid + "&ListId=" + listid + "&ID=" + itemid + "&subaction=" + subaction + "Source=" + document.location.href;
...
dhtmlXGridObject.prototype.hovermenushow = function (dd, cell) {
    ...
    var weburl = siteurl + "/_layouts/epmlive/gridaction.aspx?action=GetContextMenus&grid=" + this._gridid + "&webid=" + webid + "&listid=" + listid + "&id=" + itemid + "&rollups=" + this._brollups + "&requestlist=" + this._requestList;
    ...
    menu.style.top = (dd.offsetParent.offsetTop + dd.offsetParent.offsetParent.offsetTop + row.offsetHeight) + "px";
...
eXcell_tree.prototype.setValue = function (valAr) {
    ...
    if ((this.grid._tgc.imgURL == null) || (this.grid._tgc.imgURL != this.grid.imgURL)) {
        ...
        if (this.grid._enableCMenus) {
            ...
            _tgc.menu = "<DIV id='menuDiv' style='HEIGHT: 22px; MARGIN: 0px; float:right; position:absolute; top:0px; right:0px; z-index: 10; ' onclick=\"this." + (_isKHTML ? "" : "parentNode.") + "parentNode.parentNode.parentNode.parentNode.grid.hovermenushow(this, " + this.cell._cellIndex + ");\" onmouseover='return false;'><SPAN>&nbsp;</SPAN><A id='menuA' onfocus='return false;' title='Open Menu' onclick='return false;' href='javascript:;'><IMG id='menuImg' style='visibility: hidden' alt='Open Menu' src='/_layouts/images/ecbarw.png' width=7 height=4 border=0></A><SPAN>&nbsp;</SPAN></DIV>";
        }
        else {
