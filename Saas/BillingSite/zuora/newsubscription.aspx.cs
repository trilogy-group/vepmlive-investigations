...
namespace BillingSite.zuora
{
    public partial class newsubscription : System.Web.UI.Page
    {
        ...
        private void ProcessRatePlanCharge(ZuoraHelper zuora, RatePlanCharge rpc, bool bIsUltimate, int version, string SKU)
        {
        ...
        private void ProcessRatePlan(ZuoraHelper zuora, RatePlan rp, int version)
        {
            ...
            if(qrPRP.records.Length > 0 && qrPRP.records[0] != null)
            {
                ...
                if(qrP.records.Length > 0 && qrP.records[0] != null)
                {
                    ...
                    QueryResult qrRPC = zuora.RunQuery("select Quantity,ProductRatePlanChargeId, Name, AccountingCode,Version,EffectiveStartDate,EffectiveEndDate from RatePlanCharge where RatePlanId = '" + rp.Id + "' and Quantity > 0");

                    for(int i = 0; i < qrRPC.records.Length; i++)
                    {
                        if(qrRPC.records[i] != null)
                        {
                            ProcessRatePlanCharge(zuora, (RatePlanCharge)qrRPC.records[i], bIsUltimate, version, product.SKU);
                        }
                    }
                }
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if(qrSub.records.Length > 0 && qrSub.records[0] != null)
            {
                ...
                ContractStart = sub.ContractEffectiveDate.Value;
                ...
                if(sub.ContractEffectiveDateSpecified)
                {
                    ...
                    for(int i = 0; i < qrRP.records.Length; i++)
                    {
                        ...
                        {
                            ProcessRatePlan(zuora, (RatePlan)qrRP.records[i], version);
                        }
                    }
