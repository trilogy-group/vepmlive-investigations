...
begin
	...
	FROM         dbo.ACCOUNT_EXTERNAL_SITES INNER JOIN
						  ...
						  dbo.ORDERDETAIL.detail_type_id = dbo.ACCOUNT_USERS.DETAIL_ID
	WHERE     (dbo.ACCOUNT_EXTERNAL_SITES.siteguid = @siteid) AND (dbo.ORDERS.expiration > GETDATE())
	AND (dbo.ACCOUNT_USERS.deleted IS NULL)
	AND NOT EXISTS (SELECT 1 FROM dbo.ACCOUNT_USER_EXCLUDE WHERE dbo.ACCOUNT_USER_EXCLUDE.[uid] = dbo.ACCOUNT_USERS.[user_id])
